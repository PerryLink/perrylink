...
<!-- Round rotation: keep only the newest two rounds (current + one previous). Older rounds live in git history and can be restored on request. Rounds are narrative only — no Snapshot/counter lines: live counts belong to the badge row and "Where the plugins live", and per-edition copies go stale. GHSA/long-lived references belong in "Upstream & community contributions".
     ...
     ROUND COUNT, 2026-09-24: both blocks carry two rounds -- 09-24 (current) and 09-23
     (previous). The 09-22 English round rotated out to git history by the rule above. The
     09-24 round is a compression, not a new narrative: the paper it used to lead with moved
     into the page intro, where a DOI reference belongs, and the laya history plus the
     claude-agent-acp fix are kept at one sentence each. The earlier 09-20 (Chinese) and 09-21
     (both) rounds were deleted at the owner's request. Consequence to keep in mind, not a
     broken link: the 09-21 round was where laya-mcp and dsh-laya were introduced by name.
     dsh-laya still has its family-table row (the sidecar-client design lives there), and
     laya-mcp carries a clause in the page intro; the design rationale that round held -- why
     the sidecar exists at all -- is the one thing that went with it and was not restored. -->
...
- **The research this family rests on became a paper with a DOI — a measurement, in large part, of two projects on this page.** [*When a Judgment Layer's Self-Reported Fields Lie: Cost, Latency and the Failure Boundary of Three Judgment Layers on the Same Items*](https://doi.org/10.5281/zenodo.22901853) measures Laya, TypeSafe Jev and DeepSeek-V4.1-Flash on one item set: four claims, **three hold and one the data refutes**. The judges' self-reported access-layer fields are not trustworthy — a truncation flag reporting `passed` while dropping input, a probability field that inverts an item's conclusion, two verdict words unreachable under real input — and the shape behind them is that the judge is near-perfect (0.9909, n=220) where the answer is explicit and collapses (0.3091, n=220) where it must notice an **absence**, with self-reported confidence high in both cases. A dissimilar judge supplies **no** incremental coverage in any of the three regimes. Links, and the rule for citing the English text and its [Chinese translation](https://doi.org/10.5281/zenodo.22902025) as one work rather than two, are in the intro above; the [artifact](https://doi.org/10.5281/zenodo.22901248) is a third record.

- **Nine of my pull requests landed in [laya](https://github.com/NandhaKishorM/laya) that day, eight of them inside 84 seconds** (13:22:52Z–13:24:16Z) — the project's first Windows CI lane ([#212](https://github.com/NandhaKishorM/laya/pull/212)), a suite for the load-time and budget errors nothing reached ([#237](https://github.com/NandhaKishorM/laya/pull/237)), three assertions that could not fail ([#231](https://github.com/NandhaKishorM/laya/pull/231)), inference moved off the event loop ([#230](https://github.com/NandhaKishorM/laya/pull/230)), an ECE that binned differently from its own siblings ([#232](https://github.com/NandhaKishorM/laya/pull/232)), non-ASCII characters surviving non-string instructions ([#228](https://github.com/NandhaKishorM/laya/pull/228)) and a disclaimer heuristic that read the word "confidential" as one ([#227](https://github.com/NandhaKishorM/laya/pull/227)), with [#211](https://github.com/NandhaKishorM/laya/pull/211), [#210](https://github.com/NandhaKishorM/laya/pull/210) and [#169](https://github.com/NandhaKishorM/laya/pull/169) joining them; [#222](https://github.com/NandhaKishorM/laya/pull/222), [#234](https://github.com/NandhaKishorM/laya/pull/234), [#249](https://github.com/NandhaKishorM/laya/pull/249) and [#299](https://github.com/NandhaKishorM/laya/pull/299) followed before the next round — **[16 merged in total](https://github.com/NandhaKishorM/laya/pulls?q=is%3Apr+author%3APerryLink+is%3Amerged), not one closed unmerged.** The maintainer's note on [#210](https://github.com/NandhaKishorM/laya/pull/210) is the one worth keeping: **"it is better for having been written by someone outside the project."**

- **And one elsewhere: a process that could not start at all now starts.** [claude-agent-acp #1146](https://github.com/agentclientprotocol/claude-agent-acp/pull/1146) fixes [issue #1124](https://github.com/agentclientprotocol/claude-agent-acp/issues/1124) — `src/index.ts` awaited `resolveSettings` at module scope with nothing around it, so a transient `EINTR`/`EMFILE`/`EAGAIN` while reading the managed-policy tier rejected a top-level await in an ESM entry point and exited the process before it served a single ACP message; the client saw exit 1 and silence. The fix extracts the read into `src/managed-policy.ts` — for one reason, that an ESM entry point with top-level side effects cannot be imported by a test — and adds four regression tests, the decisive one asserting the promise **resolves** with the target env untouched. `tests` went from `17 failed | 1366 passed` to `17 failed | 1370 passed`; the 17 pre-existing failures are recorded as untouched and explicitly not claimed either way.

## 🚀 Flagship picks (start here)

*The six most-starred family plugins (★ measured 2026-09-24); every other family repo is listed in full further down, and the research four-piece set is under [Research](#-research-4).*

| Plugin | What it gives you | Install |
|---|---|---|
| [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) | Second-model auto-review on the approval chain, fail-closed by default (198★) | `dsh plugin --profile web add dsh-auto-review` |
| [dsh-research-report](https://github.com/PerryLink/dsh-research-report) | Verifiable research reports: content-addressed evidence ledger, manifest seal hash, byte-level citation checks, drift detection, disproof ledger (158★) | `dsh plugin --profile web add dsh-research-report` |
| [dsh-industry-research](https://github.com/PerryLink/dsh-industry-research) | Industry/company research: chain-map SVG with bottleneck detection, timeline, company cards, adversarial review (157★) | `dsh plugin --profile web add dsh-industry-research` |
...
| [dsh-memento](https://github.com/PerryLink/dsh-memento) | Approval-gated cross-session memory (`ctx.memory` + SQLite) (117★) | `dsh plugin --profile web add dsh-memento` |
...
| [laya](https://github.com/NandhaKishorM/laya) | 22,425 | NandhaKishorM — individual maintainer; the repo was created 2026-09-18 |
...
- **Correctness and test repairs** — three assertions that could not fail ([#231](https://github.com/NandhaKishorM/laya/pull/231)), the load-time and budget errors no suite reached ([#237](https://github.com/NandhaKishorM/laya/pull/237)), the test suite run on Windows as well as Linux ([#212](https://github.com/NandhaKishorM/laya/pull/212)), an ECE that binned differently from its siblings ([#232](https://github.com/NandhaKishorM/laya/pull/232)), non-ASCII characters kept in non-string instructions ([#228](https://github.com/NandhaKishorM/laya/pull/228)), a README link pointing at a heading that does not exist ([#236](https://github.com/NandhaKishorM/laya/pull/236)), [#249](https://github.com/NandhaKishorM/laya/pull/249), where a `noul` criteria dict that cannot be read raises instead of silently falling back to defaults — the line the project's 0.3.11 release note calls "stricter noul criteria" — and [#299](https://github.com/NandhaKishorM/laya/pull/299), two parity cells in the benchmark table that did not match the JSON they cite.
...
**laya**([NandhaKishorM/laya](https://github.com/NandhaKishorM/laya))是这个账号投入最深的外部项目:**16 条已合并的 PR** —— 我在那里提的 PR 全部进了主干,没有一条被关闭未合并;外部贡献者里最多,仅次于直接向 `main` 提交(而非走 PR)的维护者。其中 9 条是 2026-09-23 合的,8 条集中在 84 秒之内(13:22:52Z–13:24:16Z);[#222](https://github.com/NandhaKishorM/laya/pull/222)、[#234](https://github.com/NandhaKishorM/laya/pull/234)、[#249](https://github.com/NandhaKishorM/laya/pull/249) 与 [#299](https://github.com/NandhaKishorM/laya/pull/299) 随后进主干。方向不是一个,而是四个:多语言路由与评测、HTTP 服务与容器、邮件免责声明与语种判定,以及代码与测试修正 —— 包括项目的**第一条 Windows CI 车道**([#212](https://github.com/NandhaKishorM/laya/pull/212)),Linux 的 16 个 suite 里 15 个现在在 `windows-latest` 上跑。
