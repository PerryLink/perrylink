...
<!--
  ...
  live profile 2026-09-21 (16px margin + ~5px descent). One <p> drops the margin and leaves the
  descent alone: re-measured on that page after the change, 5px -- a fifth of the old gap. The profile
  renders this README at a 14px/21px line box while the file view uses 16px/24px, so anything tied to
  the line box lands differently on each surface: align="top" on the images measured 1px on the
  profile against 4px in the file view, tighter than the rows want, and was left off for that reason.
...
<a href="https://github.com/PerryLink/dsh-plugin-doctor#readme"><img alt="OpenSSF Scorecard" src="https://img.shields.io/ossf-scorecard/github.com/PerryLink/dsh-auto-review?label=openssf%20scorecard"></a><br>
...
## 📣 Latest — 2026-09-22 round

- **Four upstream pull requests merged, and one of them waited on its own CI.** [reactive-resume](https://github.com/reactive-resume/reactive-resume) [#3527](https://github.com/reactive-resume/reactive-resume/pull/3527) — the follow-up that rejects out-of-range months which rendered an employment period as `undefined 2019 - Present` — merged after the maintainer's approval and a green `e2e` run; [laya](https://github.com/NandhaKishorM/laya) [#94](https://github.com/NandhaKishorM/laya/pull/94) — a disclaimer footer sharing a paragraph made `clean_email_body` delete the whole body — merged with its new regression suite wired into CI in the same change; and [walkinglabs' plugin list](https://github.com/walkinglabs/awesome-deepseek-harness-plugins) took [#76](https://github.com/walkinglabs/awesome-deepseek-harness-plugins/pull/76) and [#65](https://github.com/walkinglabs/awesome-deepseek-harness-plugins/pull/65), adding 36 family plugins to its curated README.
- **Upstream shipped 0.1.7-alpha.1, and it deleted the switch that a source launch used to depend on.** The release removes `resolutionMode` outright — `profile-prompt.ts` now calls `createRuntimeResolution` unconditionally — so a source/`pnpm` launch no longer has two resolvers to disagree about. Worth recording because two of the three threads reporting *"Cannot read properties of undefined (reading 'prepare')"* were source launches, and the underlying defect was two module instances of `@deepseek-ai/dsh-tools` (the emitted `lib/index.js` and `src/index.ts`) each holding its own private `TOOL_RUNTIME_SCHEDULER` symbol.
- **Eighteen unanswered Discussions threads answered, and the mechanism written up where it was missing.** Upstream answers now carry the two-resolver root cause with file-and-line references, and one thread that had gone three days under the title *"Has no one updated this bug"* got both the cause and the fix commit.
- **The day's other half went outward: eight open pull requests and two issues against other vendors' agent repositories, on the rule that a proposal is not finished until it is verified at the level the maintainer will verify it.** [BrowserSkill](https://github.com/Tencent/BrowserSkill) #313 · [agent-browser](https://github.com/vercel-labs/agent-browser) #1969 and issue #1970 · [teamai-cli](https://github.com/Tencent/teamai-cli) #695 · [deepsec](https://github.com/vercel-labs/deepsec) #177 · [codex-security](https://github.com/openai/codex-security) #986 · [tau](https://github.com/huggingface/tau) #734 · [docker-agent](https://github.com/docker/docker-agent) #4372 · [agent-client-protocol](https://github.com/agentclientprotocol/agent-client-protocol) #2191/#2192. In practice that meant re-running each repo's own gates locally (`pnpm lint` and `knip`, `ruff check` and `format --check`, `go test`, `cargo test`, all four deepsec CI gates), and running the *documented* test command rather than a convenient one — which is how issue #1970 was found at all.
- **Two defects were caught only by going past the unit tests.** The teamai-cli `qoder-cn` change passed its own suite and its type check and still shipped a real one: hooks landed in `~/.qoder/settings.json`, the *international* install's file, because `resolveHookScope` maps a non-self project scope to `HOME` while every call site derived its paths from the config's scope. Driving the built binary against the public `teamai-hub` dogfood repo with a scratch `HOME` showed it in one run and turned up four call sites with the same mismatch. Then fixing that mapping made two targets claim the same project-scope hook file, so a plain Qoder user's config was rewritten with `--tool qoder-cn` and lost its tool-scoped hooks — reproduced first, then pinned by a test that fails if the ownership rule is removed.
...
*The six most-starred family plugins (★ measured 2026-09-22); every other family repo is listed in full further down.*
...
**★ 1,000+ — named individually, as the rule requires.** Eleven external repos above a thousand stars carry merged work:

- **[reactive-resume](https://github.com/reactive-resume/reactive-resume) (43,257★)** — merged [#3515](https://github.com/reactive-resume/reactive-resume/pull/3515): a Kubernetes self-hosting guide, plus a follow-up fix rejecting out-of-range months that rendered as `"undefined"` ([#3527](https://github.com/reactive-resume/reactive-resume/pull/3527), merged 2026-09-21).
...
- **[laya](https://github.com/NandhaKishorM/laya) (14,052★)** — merged [#94](https://github.com/NandhaKishorM/laya/pull/94) (2026-09-21): a disclaimer footer sharing a paragraph with the body made `clean_email_body` delete the entire message. The fix is paired with a regression suite wired into CI in the same change — the repo's hard-coded test list ran two files, so the new suite would otherwise never have executed.
