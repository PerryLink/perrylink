...
- **Three more pull requests merged into [laya](https://github.com/NandhaKishorM/laya) — four in total now, which ties this account for second on its contributor list — and all three ship in its 0.3.7 release.** [#169](https://github.com/NandhaKishorM/laya/pull/169) (76% of Unicode's letters matched no script range, so text in an unlisted script was routed to the English checkpoint; the fix counts them under `other` instead, and the maintainer checked on real data that no English text changed route across 20,000 states), [#210](https://github.com/NandhaKishorM/laya/pull/210) (the reproducible per-language evaluation harness [#35](https://github.com/NandhaKishorM/laya/issues/35) asked for — per-case JSON, per-language accuracy and ECE; **the maintainer's note on it is the one worth keeping: "it is better for having been written by someone outside the project"**), and [#211](https://github.com/NandhaKishorM/laya/pull/211) (an opt-in caller-supplied language hint on `Router`, default path untouched, which the maintainer took and which made another contributor's [#204](https://github.com/NandhaKishorM/laya/pull/204) redundant). The three fixes came out of three of my own issues — [#93](https://github.com/NandhaKishorM/laya/issues/93), [#168](https://github.com/NandhaKishorM/laya/issues/168) and [#35](https://github.com/NandhaKishorM/laya/issues/35) — all of which the maintainer closed as fixed by the merged work.
- **A fourth is a CI job, and it is the first Windows lane that project has had.** [laya #212](https://github.com/NandhaKishorM/laya/pull/212) runs the whole model-free suite on `windows-latest`: 15 of the 16 suites the Linux job runs now execute there, and `test_docker_entrypoint` is skipped with the reason recorded in the workflow rather than quietly excluded. The required `tests (pyX.Y)` check names are left untouched, so nothing else in that project's CI moves. All 14 checks pass, `tests (windows)` included.
...
**laya 上又合了三个 PR,累计四个,在该项目贡献者榜上并列第二,三个都进了它的 0.3.7 版本。** [#169](https://github.com/NandhaKishorM/laya/pull/169)(Unicode 76% 的字母不落在任何 script 区间里,于是未列出的文字被判给英文 checkpoint;改成计入 `other`,维护者在真实数据上核过 20,000 条 state 里没有一条英文改变了路由)、[#210](https://github.com/NandhaKishorM/laya/pull/210)([#35](https://github.com/NandhaKishorM/laya/issues/35) 要的那套可复现分语言评测 harness:逐条 JSON、分语言准确率与 ECE;**维护者给它的那句评语值得留着 ——「it is better for having been written by someone outside the project」**)、[#211](https://github.com/NandhaKishorM/laya/pull/211)(`Router` 上可选的外部语言提示,默认路径不动;维护者采纳后,另一位贡献者的 [#204](https://github.com/NandhaKishorM/laya/pull/204) 因此变成冗余)。三个修复分别出自我自己提的三个 issue —— [93](https://github.com/NandhaKishorM/laya/issues/93)、[168](https://github.com/NandhaKishorM/laya/issues/168)、[35](https://github.com/NandhaKishorM/laya/issues/35) —— 维护者都以「已由合并的工作修好」关闭。

**第四个是 CI:给那个项目加上了它的第一条 Windows 车道。** [laya #212](https://github.com/NandhaKishorM/laya/pull/212) 在 `windows-latest` 上跑完整套无模型测试 —— Linux 那 16 个 suite 里有 15 个现在在 Windows 上也跑,`test_docker_entrypoint` 被跳过但把原因写进了 workflow,而不是悄悄排除;必需的 `tests (pyX.Y)` 检查名一个没动,所以那个项目 CI 的其它部分不受影响。14 项检查全绿,含 `tests (windows)`。
