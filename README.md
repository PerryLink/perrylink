...
- **[Gitee](https://gitee.com/perrylink)** — 105 public repos: all 45 family repos are mirrored (default branch + all tags, not every branch) alongside mirrors of other projects; the upgrade corridor lives there under its former name `dsh-plugin-upgrade-rc1`
- **npm** — 41 packages published through CI (36 unscoped + 5 scoped; 40 carry a provenance attestation on their current latest version), plus 4 deprecated old names still on the registry (the two superseded corridor legs `dsh-plugin-upgrade` and `dsh-plugin-upgrade-rc1`, the withdrawn `dsh-personal-directive`, and the retired scoped `@perrylink/dsh-cert-mcp`, renamed to the unscoped `dsh-cert-mcp`) — 45 names, 734 versions (768 with pan17's dsh-wechat) — 69.1k downloads in August plus 82.4k in September through the 18th (131.0k over the trailing 30 days, npm window 08-20..09-18 — the series zeroes out 09-03, 09-07, 09-08, 09-15 and 09-17; the 45 PerryLink names alone are 63.9k / 79.8k / 125.7k)
