...
![stars](https://img.shields.io/badge/stars-1256-yellow)
...
- **The 2026-09-19 wave was a gate-and-release day: 190 commits and 53 Releases across 27 family repos.** The CI work is the part that outlives it — the published-line type ruler reached CI in **18 repos**, **41** capped the compat-profile job at 25 minutes, **22** added a fail-fast when `package.json` and the lockfile disagree, and **4** stopped their "second ruler" from measuring the same type universe as the first.
- **Releases worth opening:** [dsh-plugin-upgrade](https://github.com/PerryLink/dsh-plugin-upgrade) **2.0.0** folds three retired corridor legs into one package with corridor-index routing (2.0.1 followed on the 20th), [dsh-cert-mcp](https://github.com/PerryLink/dsh-cert-mcp) **0.2.0–0.2.2** moved off the `@perrylink/` scope and onto the official MCP Registry, [dsh-draw](https://github.com/PerryLink/dsh-draw) **0.2.15** and dsh-plugin-upgrade now publish through npm trusted publishing (OIDC), and six repos — [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) among them — made the post-publish registry check advisory; the rest of the 27-repo wave was routine hygiene and dependency bumps.
- **The 0.1.6-alpha.2 upgrade corridor finished landing in the small hours** — the batch PRs merged across the fleet, `dsh-typert-protocol` was pinned to one line, and three repos regenerated lockfiles their peer-range edits had left stale.
...
|---|---|---|
| [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) | Second-model auto-review on the approval chain, fail-closed by default (187★) | `dsh plugin --profile web add dsh-auto-review` |
...
| [dsh-research-report](https://github.com/PerryLink/dsh-research-report) | Verifiable research reports: content-addressed evidence ledger, manifest seal hash, byte-level citation checks, drift detection, disproof ledger (135★) | `dsh plugin --profile web add dsh-research-report` |
...
- **GitHub** (this profile) — source, CI, releases; **1,256★ across all 165 public repos, 1,186★ of it in the 45 family repos** (40 `dsh.bundle` plugins + 5 support repos; the 44 PerryLink-owned ones are 1,178★, the other 8★ is [pan17/dsh-wechat](https://github.com/pan17/dsh-wechat)), 43 of the PerryLink-owned repos declaring the `dsh.bundle` contract
...
- **npm** — 41 active packages published through CI (36 unscoped + 5 scoped), of which 40 carry a provenance attestation on their current latest version — the lone exception is `@perrylink/dsh-plugin-doctor`; across all 45 published names the attestation count is 44 — plus 4 deprecated old names still on the registry (the retired corridor legs `dsh-plugin-upgrade-rc1` and `dsh-plugin-upgrade-015`, the withdrawn `dsh-personal-directive`, and the retired scoped `@perrylink/dsh-cert-mcp`, renamed to the unscoped `dsh-cert-mcp`) — 45 names, 736 versions (770 with pan17's dsh-wechat) — 69.1k downloads in August plus 82.4k in September through the 18th (131.0k over the trailing 30 days, npm window 08-20..09-18 — the series zeroes out 09-03, 09-07, 09-08, 09-15 and 09-17; the 45 PerryLink names alone are 63.9k / 79.8k / 125.7k)
...
我在 DeepSeek Harness 上把自己希望存在的插件做了出来:检查点回退、声明式权限、输出风格、跨会话记忆、MCP 面板、模型档位路由、微信/TickTick 桥接,以及可验证研究报告 / 基金研究 / 行业研究 / 数据质量「研究四件套」——共 **40 个插件**(Apache-2.0):全部带 CI、npm 发布与 Gitee 镜像,45 个家族仓中有 40 个带五语文档,43 个 PerryLink 自有仓声明 `dsh.bundle` 契约。npm 已发布 **41 个在用包**(36 个非 scoped + 5 个 scoped),其中 40 个当前 latest 版本带 provenance 证明(唯一例外是 `@perrylink/dsh-plugin-doctor`;45 个已发布名称合计 44 个带证明),另有 4 个弃用旧名仍挂在 registry 上;45 个名称累计 736 个版本(含 pan17 的 dsh-wechat 为 770 个),下载量 8 月 69.1k + 9 月截至 18 日 82.4k(近 30 天 131.0k;npm 日序列在 09-03、09-07、09-08、09-15、09-17 归零)。一键全家桶:[`dsh-kit`](https://github.com/PerryLink/dsh-kit)。同时我是 DeepSeek Harness 上游核心框架 **Cordis**([cordiverse/cordis](https://github.com/cordiverse/cordis))的贡献者,也是 [deepseek-ai](https://github.com/deepseek-ai) 官方仓贡献者:[FlashMLA](https://github.com/deepseek-ai/FlashMLA) 的修复已合并(#224);逐条 PR/issue 见下方 Upstream 一节。

2026-09-20 轮:① **2026-09-19 是一个「门禁 + 发布」日——27 个家族仓共 190 次提交、53 个 Release**。真正留得下来的是 CI 部分:「已发布线」类型尺子进入 **18 个仓**的 CI,**41** 个仓把 compat profile 封顶 25 分钟,**22** 个仓加上「`package.json` 与 lockfile 不一致即失败」,**4** 个仓让「第二把尺子」不再量与第一把同一个类型宇宙。② **值得点开的版本**:[dsh-plugin-upgrade](https://github.com/PerryLink/dsh-plugin-upgrade) **2.0.0** 把三条已退役的走廊腿折成一个包 + 走廊索引路由(2.0.1 于 20 日跟上)、[dsh-cert-mcp](https://github.com/PerryLink/dsh-cert-mcp) **0.2.0–0.2.2** 去掉 `@perrylink/` scope 并进入官方 MCP Registry、[dsh-draw](https://github.com/PerryLink/dsh-draw) **0.2.15** 与 dsh-plugin-upgrade 改用 npm trusted publishing(OIDC)、含 [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) 在内的 6 个仓把发布后 registry 检查改为 advisory;同批 27 个仓其余为例行卫生与依赖升级。③ **0.1.6-alpha.2 升级走廊在凌晨收尾**——批量 PR 全族合并、`dsh-typert-protocol` 收敛到单一版本线、3 个仓重新生成了被 peer 区间改动遗留的 lockfile。
