...
![repos](https://img.shields.io/badge/repos-156-green)
![stars](https://img.shields.io/badge/stars-1236-yellow)
...
**Building the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) plugin ecosystem: 40 open-source plugins (`dsh.bundle` plugins) — 45 repos in the family (Apache-2.0) across security, workflows, research, messaging bridges, and developer experience — plus the DSH Desktop Market catalog, a plugin-certification registry, and the dsh-plugin-doctor CI checker. 40 of the 45 repos ship five-language READMEs, every plugin ships CI, npm provenance, and a Gitee mirror, and 42 of the PerryLink-owned repos declare the `dsh.bundle` contract — 69.1k npm downloads in August plus 82.1k in September through the 18th (130.8k over the trailing 30 days, npm window 08-20..09-18 — npm's daily series currently has gaps on 09-15 and 09-17; measured 2026-09-19). I also contribute upstream to [Cordis](https://github.com/cordiverse/cordis) — the plugin-core framework DeepSeek Harness itself is built on — and to [deepseek-ai](https://github.com/deepseek-ai) projects, with a merged [FlashMLA](https://github.com/deepseek-ai/FlashMLA) fix among them.**
...
- **The fleet stopped publishing with long-lived npm tokens.** The release workflow now takes `id-token: write` and lets the OIDC token replace `NPM_TOKEN` outright — `pnpm publish --provenance` stays — so the stale-secret failure mode the previous round found is deleted rather than patched. 23 repos touched their publish path, and the day's own wave was **138 commits across 43 repos and 25 releases** (18:42–21:15), on top of **194 commits across 45 repos and 48 releases since local midnight** — 23 of those releases were the 00:06–00:24 alpha.2 burst.
- **G-4b, the gate hardening, is now the default.** The published-line ruler runs in CI in **15 more repos** and **41** capped the compat-profile job at 25 minutes; the "second ruler" that measured the same type universe as the first was split into two real ones ([dsh-mask](https://github.com/PerryLink/dsh-mask) 0.2.12, [dsh-session-sync](https://github.com/PerryLink/dsh-session-sync) 0.2.16, [dsh-translate](https://github.com/PerryLink/dsh-translate) 0.2.12, [dsh-skill-pack-security](https://github.com/PerryLink/dsh-skill-pack-security)), and **22 repos** added a fail-fast check for a `package.json`/lockfile disagreement.
- **The honest rulers kept finding work:** [dsh-claude-move](https://github.com/PerryLink/dsh-claude-move) repaired **40** published-line type errors and [dsh-catalog](https://github.com/PerryLink/dsh-catalog) **54** — both now green in CI instead of skipped.
- **Releases worth opening:** [dsh-cert-mcp](https://github.com/PerryLink/dsh-cert-mcp) **0.2.1** dropped the `@perrylink/` scope, deprecated the old scoped name and started publishing `server.json` to the official MCP Registry; [dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) **0.12.6** ([#48](https://github.com/PerryLink/dsh-auto-review/pull/48)) moved its publish job onto OIDC — npm did receive 0.12.6, but the post-publish registry check gave up after ~30 seconds of read-side lag and exited 1, which skipped its GitHub Release step (dsh-doublecheck 0.9.13 hit the same wall; the check is advisory from the next commit on, and both runs were re-run the same evening — v0.12.6 and v0.9.13 now have their Releases, with the npm versions untouched); [dsh-local-ai](https://github.com/PerryLink/dsh-local-ai) **0.2.12** fixed the OIDC exchange itself and now logs a trusted-publisher mismatch at verbose level; [dsh-plugin-guide](https://github.com/PerryLink/dsh-plugin-guide) **0.3.15/0.3.16** re-verified the ledger against `0.1.6-alpha.2` (`ddefc45`) and shipped the refreshed official-docs mirror; [dsh-permission-rules](https://github.com/PerryLink/dsh-permission-rules) **0.7.4** and [dsh-memento](https://github.com/PerryLink/dsh-memento) **0.5.14** carried the routine hygiene that rode along in all 22 packages — the 25-minute cap, the lockfile check, the now-advisory post-publish registry check, and dependency bumps ([dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) #41–#45, [dsh-mcp-panel](https://github.com/PerryLink/dsh-mcp-panel) #31–#34).
...
- **GitHub** (this profile) — source, CI, releases; **1,236★ across all 156 public repos, 1,166★ of it in the 45 family repos** (40 `dsh.bundle` plugins + 5 support repos), 42 of the PerryLink-owned repos declaring the `dsh.bundle` contract
...
- **npm** — 41 packages published through CI with provenance (36 unscoped + 5 scoped), plus 3 deprecated old names still on the registry (the superseded corridor package `dsh-plugin-upgrade`, the withdrawn `dsh-personal-directive`, and the retired scoped `@perrylink/dsh-cert-mcp`, renamed to the unscoped `dsh-cert-mcp`) — 44 names, 730 versions (764 with pan17's dsh-wechat) — 69.1k downloads in August plus 82.1k in September through the 18th (130.8k over the trailing 30 days, npm window 08-20..09-18; the 44 PerryLink names alone are 63.9k / 79.6k / 125.4k)
...
Listed on [awesome-dsh-plugin](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin) (36 entries), [DSH Directory](https://dsh.directory/plugins?q=perrylink) (35 published), [walkinglabs' plugin list](https://github.com/walkinglabs/awesome-deepseek-harness-plugins) (Developer Tooling: [dsh-plugin-guide](https://github.com/PerryLink/dsh-plugin-guide), [dsh-plugin-upgrade-015](https://github.com/PerryLink/dsh-plugin-upgrade-015), [dsh-test-drive](https://github.com/PerryLink/dsh-test-drive)), [Awesome DeepSeek Harness](https://github.com/0xsline/awesome-deepseek-harness), [awesome-deepseekharness](https://github.com/awesome-deepseekharness/awesome-deepseek-harness), [Zhiyuan-Fan's list](https://github.com/Zhiyuan-Fan/Awesome-DeepSeek-Harness-Plugins), the [AdamPlatin123 radar](https://github.com/AdamPlatin123/dsh-plugin-radar), [dsh-market](https://github.com/dsh-market/dsh-market) (via the awesome-dsh-plugin registry), [dsh-suite](https://github.com/whyihaveyou/dsh-suite), and [ZeroPointRepo's list](https://github.com/ZeroPointRepo/awesome-dsh-plugins). Auto-indexed on [deepseek1024.com](https://deepseek1024.com) (36 entries). Certification: [dsh-plugin-certification](https://github.com/PerryLink/dsh-plugin-certification).

## 中文介绍

我在 DeepSeek Harness 上把 Claude Code / Codex 生态里最实用的能力做成了开源插件:检查点回退、声明式权限、输出风格、跨会话记忆、MCP 面板、模型档位路由、微信/TickTick 桥接,以及可验证研究报告 / 基金研究 / 行业研究 / 数据质量「研究四件套」——共 **40 个插件**(Apache-2.0),全部带五语文档、CI、npm 发布与 Gitee 镜像(45 个家族仓中有 40 个带五语文档,42 个 PerryLink 自有仓声明 `dsh.bundle` 契约)。npm:已发布 **41 个包**(36 个非 scoped + 5 个 scoped),另有 3 个仍挂在 registry 上的弃用旧名(被取代的走廊包 `dsh-plugin-upgrade`、已撤回的 `dsh-personal-directive`,以及改名为非 scoped 后退休的 `@perrylink/dsh-cert-mcp`),44 个 PerryLink 名称累计 730 个版本(含 pan17 的 dsh-wechat 为 764 个)。下载量:8 月 69.1k + 9 月截至 18 日 82.1k(近 30 天 130.8k,统计窗口 08-20..09-18;仅 44 个 PerryLink 名称为 63.9k / 79.6k / 125.4k;npm 日序列目前缺 09-15 与 09-17)。一键全家桶:[`dsh-kit`](https://github.com/PerryLink/dsh-kit)。同时我也是 DeepSeek Harness 上游核心框架 **Cordis**([cordiverse/cordis](https://github.com/cordiverse/cordis))的贡献者:Windows/macOS CI 已合并(#122),为 include-journal 修复(#121)提供了 45 个真实补丁文件的回归套件与验证,tarball 打包修复 PR(#126)与 NodeNext 类型声明 issue(#124)在途,并向 schemastery 提交了 ESM 消费面 issue(#77)与两枚 PR(#78/#79)。同时我已成为 [deepseek-ai](https://github.com/deepseek-ai) 官方仓贡献者:[FlashMLA](https://github.com/deepseek-ai/FlashMLA) #224 已合并(修复 Windows 下 sm90 st.async PTX 传输的 128 位宽度静默截断)。

2026-09-19 轮:① **全族不再用长效 npm token 发布**——发布工作流带上 `id-token: write`,OIDC 令牌直接取代 `NPM_TOKEN`(`pnpm publish --provenance` 保留),上一轮查出的「陈旧密钥」故障模式是被删掉而不是打补丁:23 个仓动了发布链路,今天一天 **43 个仓 138 次提交、25 个 Release**(18:42–21:15),自当地零点起合计 **45 个仓 194 次提交、48 个 Release**(其中 23 个是 00:06–00:24 的 alpha.2 那一波)。② **G-4b 门禁加固成为默认**——「已发布线」尺子又进了 **15 个仓**的 CI,**41 个仓**把 compat profile 任务封顶 25 分钟;上一轮查出「与第一把量同一个类型宇宙」的第二把尺子被拆成两把真尺子([dsh-mask](https://github.com/PerryLink/dsh-mask) 0.2.12、[dsh-session-sync](https://github.com/PerryLink/dsh-session-sync) 0.2.16、[dsh-translate](https://github.com/PerryLink/dsh-translate) 0.2.12、[dsh-skill-pack-security](https://github.com/PerryLink/dsh-skill-pack-security)),**22 个仓**新增「`package.json` 与 lockfile 不一致即失败」的检查。③ **尺子变诚实后继续抓到活**:[dsh-claude-move](https://github.com/PerryLink/dsh-claude-move) 修掉 **40** 个已发布线类型错误、[dsh-catalog](https://github.com/PerryLink/dsh-catalog) 修掉 **54** 个,两仓 CI 从跳过变绿。④ **值得点开的版本**:[dsh-cert-mcp](https://github.com/PerryLink/dsh-cert-mcp) **0.2.1** 去掉 `@perrylink/` scope、给旧 scoped 名打上改名弃用,并开始把 `server.json` 发到官方 MCP Registry;[dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) **0.12.6**([#48](https://github.com/PerryLink/dsh-auto-review/pull/48))把发布任务切到 OIDC——npm 上 0.12.6 确实已就位,但发布后的 registry 校验只等了约 30 秒就退出 1,于是 GitHub Release 这一步被跳过(dsh-doublecheck 0.9.13 撞上同一堵墙;该检查自下一个提交起改为建议性,两条 run 当晚已重跑,两个 Release 均已补出,npm 侧未受影响);[dsh-local-ai](https://github.com/PerryLink/dsh-local-ai) **0.2.12** 修好 OIDC 交换本身,并在 verbose 级别打印 trusted-publisher 不匹配;[dsh-plugin-guide](https://github.com/PerryLink/dsh-plugin-guide) **0.3.15/0.3.16** 对 `0.1.6-alpha.2`(`ddefc45`)重新验证并发出刷新的 official-docs 镜像;[dsh-permission-rules](https://github.com/PerryLink/dsh-permission-rules) **0.7.4**、[dsh-memento](https://github.com/PerryLink/dsh-memento) **0.5.14** 等 22 个包照例行卫生:25 分钟封顶、lockfile 检查、改为建议性的发布后 registry 检查,以及依赖升级([dsh-auto-review](https://github.com/PerryLink/dsh-auto-review) #41–#45、[dsh-mcp-panel](https://github.com/PerryLink/dsh-mcp-panel) #31–#34)。

### 2026-09-18 轮

① **0.1.6-alpha.2 升级波——23 个插件一次串行发完**：全族 peer 区间补上第三段 `>=0.1.6-0 <0.2.0`,清单统一声明 `dsh.manifestVersion: 1` + `engines.dsh`,23 个包带 provenance 依次发布到 npm([dsh-local-ai](https://github.com/PerryLink/dsh-local-ai) 0.2.11、[dsh-score](https://github.com/PerryLink/dsh-score) 0.2.12、[dsh-memento](https://github.com/PerryLink/dsh-memento) 0.5.13、[dsh-talk](https://github.com/PerryLink/dsh-talk) 0.3.11、[dsh-fast](https://github.com/PerryLink/dsh-fast) 0.2.13、[dsh-observe](https://github.com/PerryLink/dsh-observe) 0.2.13、[dsh-click](https://github.com/PerryLink/dsh-click) 0.3.11、[dsh-cert-mcp](https://github.com/PerryLink/dsh-cert-mcp) 0.1.7 等);② **门禁本身被审计出两类系统性问题**——全族排查发现 **18 个仓的「已发布线」类型尺子从未在 CI 里跑过**、**11 个 JS 仓的第二把尺子量与第一把同一个类型宇宙**(`paths: {}` 是空操作);两类都已修或已排期,而新尺子立刻抓到真缺陷(附件契约改名、四个陈旧传递 peer 让 built entry 直接无法 import、重复的 `typert-protocol` 代际只有 `skipLibCheck: false` 看得见);③ **发布链路也修了三处**:全族陈旧的 `NPM_TOKEN`(npm 对未授权发布返回 **404** 而非 401)、peer 区间改动后遗留的 lockfile、以及早于 `agent/created` 带上 `source`/`signal` 的 compat pin;④ **12 个面向客户端的插件按住不发**,等人工浏览器清单标记 VERIFIED。
